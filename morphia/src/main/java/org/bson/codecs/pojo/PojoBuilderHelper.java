...
public final class PojoBuilderHelper {
    ...
    static <T> void configureClassModelBuilder(final ClassModelBuilder<T> classModelBuilder, final Class<T> clazz) {
        ...
        while (!currentClass.isEnum() && currentClass.getSuperclass() != null) {
            ...
            for (Field field : currentClass.getDeclaredFields()) {
                ...
                final TypeData<Object> typeData = (TypeData<Object>) TypeData.newInstance(field);
                PropertyMetadata<?> propertyMetadata = getOrCreateFieldPropertyMetadata(field.getName(), declaringClassName,
                        propertyNameMap, typeData, propertyTypeParameterMap, parentClassTypeData, genericTypeNames,
                ...
                FieldMetadata<?> fieldMetadata = new FieldMetadata<Object>(field, typeData);
                ...
                classModelBuilder.addField(createFieldModelBuilder(fieldMetadata));
            }

            parentClassTypeData = TypeData.extend(parentClassTypeData, genericTypeNames, currentClass.getGenericSuperclass(), currentClass
                                                 );
            annotations.addAll(extractAnnotations(currentClass.getInterfaces()));
    ...
    private static List<? extends Annotation> extractAnnotations(final Class<?>[] interfaces) {
        final List<Annotation> annotations = new ArrayList<Annotation>();
        Set<Class<?>> seen = new LinkedHashSet<Class<?>>();

        final List<Class<?>> classes = new ArrayList<Class<?>>(asList(interfaces));
        ListIterator<Class<?>> iterator = classes.listIterator();
        while (iterator.hasNext()) {
            final Class<?> klazz = iterator.next();
            if(!seen.contains(klazz)) {
                annotations.addAll(asList(klazz.getDeclaredAnnotations()));
                for (final Class<?> aClass : klazz.getInterfaces()) {
                    iterator.add(aClass);
                }
            }
    ...
    public static <T, S> PropertyMetadata<T> getOrCreateMethodPropertyMetadata(final String propertyName,
    ...
    private static <V> void specializeFieldModelBuilder(final FieldModelBuilder<V> fieldModelBuilder, final FieldMetadata<V> metaData) {
        if (metaData.getTypeParameterMap().hasTypeParameters() && !metaData.getTypeParameters().isEmpty()) {
            ...
            if (classType != null) {
                ...
                List<TypeData<?>> parameters = new ArrayList<TypeData<?>>(fieldModelBuilder.getTypeData().getTypeParameters());
