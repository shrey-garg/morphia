...
public class ClassModelBuilder<T> {
    ...
    public ClassModel<T> build() {
        ...
        return new ClassModel<T>(type, propertyNameToTypeParameterMap, instanceCreatorFactory, discriminatorEnabled, discriminatorKey,
            discriminator, idPropertyModel, unmodifiableList(annotations), unmodifiableList(fieldModels),
            unmodifiableList(propertyModels));
    }
    ...
    public ClassModelBuilder<T> addField(final FieldModelBuilder<?> fieldModelBuilder) {
        fieldModelBuilders.add(notNull("fieldModelBuilder", fieldModelBuilder));
        return this;
    }
